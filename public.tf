...
resource "aws_subnet" "public" {
  count             = "${length(var.igw_id) > 0 ? local.subnets_count : 0}"
  vpc_id            = "${var.vpc_id}"
  ...
  tags = {
    "Name"      = "${module.public_label.id}${var.delimiter}${format("%v-%v", element(var.names, count.index), element(var.availability_zones, count.index))}"
...
locals {
  route_table_public = "${coalescelist(aws_subnet.public.*.id, list("workaround"))}"
}
...
resource "aws_route" "public" {
  count                  = "${length(var.igw_id) > 0 ? local.subnets_count : 0}"
  route_table_id         = "${element(aws_subnet.public.*.id, count.index)}"
