...
locals {
  public_route_table_id = "${coalescelist(aws_route_table.public.*.id, list("workaround"))}"
  public_subnet_ids     = "${coalescelist(aws_subnet.public.*.id, list("workaround"))}"
}
...
resource "aws_route" "public" {
  count                  = "${length(var.public_subnets_names) > 0 ? local.subnets_count : 0}"
  route_table_id         = "${element(local.public_route_table_id, count.index)}"
